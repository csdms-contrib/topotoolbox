// mexGetMats_BC.c
// [Ar Ac Av Af B] = mexGetMats_BC_v1(z,bdy,dt,dx,dy,Nx,Ny,a,b,U)
   ...
   for (j=0; j<J; j++) {
       if (i>0 && i<K-1 && j>0 && j<J-1) { // this version doesn't handle periodic or non-pinned boundaries
           ...
           if (bdy[c]!=1) { // leave f and F zero if it's a pinned element
                ...
                // Stop addition
                // calculate f
                f[c] = U[c] + a*Lap*invden + 2*ab*invden2*(zx*zx*zxx + zy*zy*zyy + 2*zx*zy*zxy);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  // syntax is [Ar Ac Av Af B] = mexGetMats_BC_v1(z,bdy,dt,dx,dy,Nx,Ny,a,b,U);
