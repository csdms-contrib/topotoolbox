...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	mwIndex ix, neigh, row, col, r, c, ixn;
    ...
    for( row= 0; row<nrow; row++ ) {
		...
		for( col= 0; col<ncol; col++) {
			...
			for ( neigh = 0; neigh < 8; neigh++ ) {
                ...
                if (isinvalidneighbor(r,c,nrow,ncol)) {
                   continue;
                }
...
// checks if a neighbor is valid. Invalid neighbors are outside the grid
int isinvalidneighbor(mwIndex r,mwIndex c,mwIndex nrow,mwIndex ncol) {    
