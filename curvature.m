...
%     ...   = curvature(DEM,res)
%     curv  = curvature(DEM,res,'type')
%     [profc,planc] = curvature(DEM,res,'both')
%
% Description
%     
%     curvature returns the second numerical derivative (curvature) of a
%     digital elevation model with cellspacing res (default = 1). 
...
%     load exampleDEM
%     d = abs(Y(1)-Y(2));
%     [profc,planc] = curvature(dem,d);
%
%     subplot(2,1,1)
%     surf(X,Y,dem,planc); axis image
%     shading interp; camlight
%     title('planform curvature')
%     subplot(2,1,2)
%     surf(X,Y,dem,profc); axis image
%     shading interp; camlight
...
validateattributes(d, {'numeric'}, {'>',0,'scalar'},'curvature', 'res')
